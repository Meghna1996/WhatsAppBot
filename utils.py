...
def fetch_reply(msg, session_id):
	...
	if response.intent.display_name == 'movie':
		...
		return news,type1,str1,news['poster']
		...
		return news,type1,str1,news[0]['poster']
	else:
		return response.fulfillment_text
def fetch_reply_favorite(msg,session_id):
	response = detect_intent_from_text(msg, session_id)
	if response.intent.display_name == 'favorite':
		favorite_string = get_favorite(response.parameters)
		return favorite_string
